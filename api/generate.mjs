...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemInstruction = `Act as a master strategist. Tone: "${tone}". ${lengthInstruction}
    REQUIRED: Generate content for: ${platforms.join(', ')}. 
    
    For each platform, provide:
    - NEWSLETTER_SUBJECT: (Newsletter only) 3 high opening rate options.
    - POST_CONTENT: Use <br><br> for spacing.
    - VISUAL_SUGGESTION: (Not for Newsletter) A brief description of an image or graphic to pair with this.
    - STRATEGIC_HASHTAGS: (Not for Newsletter) 3-5 tags.
    - CALL_TO_ACTION: A clear closing that increases engagement.
    - NEVER USE EM DASH.
    
    Return as SINGLE JSON. No markdown backticks.`;
